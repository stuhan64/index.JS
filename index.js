...
app.post('/webhook-order', async (req, res) => {
  try {
    ...
    // Build a map of Shopify variant ID -> design URL for all items in this order
    const variantDesignMap = {};
    for (const item of order.line_items || []) {
      const props = {};
      for (const p of item.properties || []) {
        props[p.name] = p.value;
      }
      const designUrl  = props['Custom Design URL'];
      const designType = props['Design Type'];
      const sunSign    = props['Sun Sign'];
      const moonSign   = props['Moon Sign'];
      const risingSign = props['Rising Sign'];

      if (designUrl) {
        variantDesignMap[item.variant_id] = {
          designUrl, designType, sunSign, moonSign, risingSign,
          title: item.title
        };
        console.log('[WEBHOOK] Item: ' + item.title + ' | variant=' + item.variant_id + ' | design=' + designType);
      }
    }

    if (Object.keys(variantDesignMap).length === 0) {
      console.log('[WEBHOOK] No custom designs found in order — skipping');
      return res.status(200).json({ received: true });
    }

    // Process as a single batch - find Printful order ONCE and update all items
    // Use first item's data for logging
    const firstItem = Object.values(variantDesignMap)[0];
    const designUrl  = firstItem.designUrl;
    const designType = firstItem.designType;
    const sunSign    = firstItem.sunSign;
    const moonSign   = firstItem.moonSign;
    const risingSign = firstItem.risingSign;

    // Dummy loop wrapper to maintain existing code structure
    for (const _once of [1]) {
      console.log('[WEBHOOK] Processing ' + Object.keys(variantDesignMap).length + ' item(s) for order ' + order.order_number);
      ...
      try {
        ...
        // Match each Printful item to the correct design using variant ID
        const updatedItems = pfItems.map(pfItem => {
          // Try to find the matching design by Shopify variant ID
          // Printful stores sync_variant_id which maps to our Shopify variant IDs
          const pfVariantId = pfItem.sync_variant_id || pfItem.variant_id;
          
          // Look up design by Printful variant -> find matching Shopify variant
          // We use our variant map in reverse to find the Shopify variant ID
          let itemDesignUrl = designUrl; // default to first item's design
          
          for (const [shopifyVariantId, itemData] of Object.entries(variantDesignMap)) {
            const printfulVid = shopifyToPrintfulVariant(shopifyVariantId);
            if (printfulVid && (pfVariantId === printfulVid || String(pfVariantId) === String(printfulVid))) {
              itemDesignUrl = itemData.designUrl;
              console.log('[WEBHOOK] Matched Printful item ' + pfItem.id + ' to design: ' + itemData.designType + ' (' + itemData.sunSign + '/' + itemData.moonSign + '/' + itemData.risingSign + ')');
              break;
            }
          }
          ...
          return {
            id:    pfItem.id,
            files: [{ type: 'front', url: itemDesignUrl }]
          };
        });
    ...
    // Still return 200 to prevent Shopify retries on parse errors
...
app.post('/webhook-order', async (req, res) => {
  try {
    ...
    for (const item of order.line_items || []) {
      ...
      if (!designUrl) {
        console.log(`[WEBHOOK] Item ${item.id} has no custom design — skipping`);
        continue;
      }
